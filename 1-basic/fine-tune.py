...
train_examples = [
    InputExample(texts=['кринж', 'что-то вызывающее смущение']),
    InputExample(texts=['масик', 'заботливый парень']),
    InputExample(texts=['тюбик','неуверенный парень']),
    InputExample(texts=['скуф', 'толстый взрослый мужчина']),
    InputExample(texts=['краш', 'привлекательный человек']),
    InputExample(texts=['чилл', 'отдых']),
    InputExample(texts=['вайб', 'атмосфера']),
    InputExample(texts=['рофл', 'шутка']),
    InputExample(texts=['флексить', 'танцевать или веселиться']),
    InputExample(texts=['хайп', 'популярность'])
]
# Создание DataLoader
train_dataloader = DataLoader(train_examples, shuffle=True, batch_size=4)

# Используем функцию потерь MultipleNegativesRankingLoss
train_loss = losses.MultipleNegativesRankingLoss(model_original)
